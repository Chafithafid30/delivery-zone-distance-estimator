...
class DeliveryApiTest {
    ...
    private String body(String ref, String address, String status) throws Exception {
        return json.writeValueAsString(
                new DeliveryRequest(
                        ref, address, status == null ? null : DeliveryStatus.valueOf(status)));
    }

    @Test
    void completeWorkflowIncludesCacheOutageFiltersUpdateRetryAndDelete() throws Exception {
        var origin =
                new GeocodingProvider.Place(new Coordinates(-6.1751, 106.8650), "Monas, Jakarta");
        when(provider.lookupAddress("monas jakarta")).thenReturn(Optional.of(origin));
        var created =
                mvc.perform(
                                post("/api/deliveries")
        ...
        when(provider.lookupAddress("surabaya")).thenThrow(new ProviderUnavailableException("503"));
        var unresolved =
                mvc.perform(
                                post("/api/deliveries")
                                        .contentType(MediaType.APPLICATION_JSON)
                        ...
                        .andExpect(jsonPath("$.distanceKm").isEmpty())
                        .andExpect(jsonPath("$.estimate").isEmpty())
                        .andReturn();
        long unknownId =
                json.readTree(unresolved.getResponse().getContentAsString()).get("id").asLong();
        ...
        doReturn(
                        Optional.of(
                                new GeocodingProvider.Place(
                                        new Coordinates(-7.2575, 112.7521), "Surabaya")))
                .when(provider)
                .lookupAddress("surabaya");
        mvc.perform(post("/api/deliveries/" + unknownId + "/geocode"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.zone").value("LONG_HAUL"));
        mvc.perform(
                        put("/api/deliveries/" + id)
                                .contentType(MediaType.APPLICATION_JSON)
                ...
                .andExpect(jsonPath("$.zone").value("LONG_HAUL"))
                .andExpect(jsonPath("$.geocodeSource").value("CACHE"));
        when(provider.lookupAddress("unavailable destination"))
        ...
        mvc.perform(
                ...
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.zone").value("UNKNOWN"))
                .andExpect(jsonPath("$.destLat").isEmpty())
                .andExpect(jsonPath("$.destLng").isEmpty())
                .andExpect(jsonPath("$.distanceKm").isEmpty())
                .andExpect(jsonPath("$.geocodedAt").isEmpty());
    ...
    @Test
    void validatesInputAndMissingIdsWithoutCallingProvider() throws Exception {
        mvc.perform(
                        post("/api/deliveries")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(body(" ", "", null)))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.errors.orderRef").exists())
                .andExpect(jsonPath("$.errors.destAddress").exists());
        mvc.perform(
                        post("/api/deliveries")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(body("X".repeat(51), "Monas", null)))
                .andExpect(status().isBadRequest());
        mvc.perform(get("/api/deliveries").param("zone", "INVALID"))
                .andExpect(status().isBadRequest());
        ...
        mvc.perform(
                        put("/api/deliveries/999999")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(body("DO-001", "Monas", null)))
