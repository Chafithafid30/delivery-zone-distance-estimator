      ...
      <div className="coordinates">
        {delivery.destLat !== null && delivery.destLng !== null ? (
          <>
            ...
            <span>
              Diambil:{" "}
              {delivery.geocodedAt ? dateTime(delivery.geocodedAt) : "—"}
            </span>
            <span>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    setListError("");
    api
      .list(zone, status, controller.signal)
      .then(setDeliveries)
      .catch((error) => {
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((result) => setFactory(result.factory))
      .catch(() => {});
    return () => controller.abort();
  }, [refresh]);
    ...
    try {
      ...
      const result =
        editing === null
          ...
          : await api.update(editing, input);
      setNotice(
        `${result.orderRef} berhasil disimpan.${result.zone === "UNKNOWN" ? " Zona belum diketahui; pengiriman tetap tersimpan." : ""}${zone || status ? " Filter aktif dapat menyembunyikan pengiriman ini." : ""}`,
      );
      resetForm();
  ...
  function edit(delivery: Delivery) {
    ...
    setForm({
      ...
      destAddress: delivery.destAddress,
      status: delivery.status,
    });
    setFormError("");
    setFieldErrors({});
    orderRefInput.current?.focus();
    orderRefInput.current?.scrollIntoView({
      behavior: "smooth",
      block: "center",
    });
  }
  ...
  async function retry(delivery: Delivery) {
    ...
    try {
      ...
      setNotice(
        result.zone === "UNKNOWN"
          ? `${result.orderRef}: koordinat masih belum tersedia. Coba kembali nanti atau perjelas alamat.`
          : `${result.orderRef}: zona berhasil diperbarui menjadi ${result.zone}.`,
      );
      setRefresh((value) => value + 1);
    } catch (error) {
      setActionError(errorMessage(error));
  ...
  return (
    <div className="app-shell">
        ...
        <div className="workspace">
          <aside className="form-panel" aria-labelledby="form-title">
            <div className="panel-heading">
              ...
              <h2 id="form-title">
                {editing === null ? "Pengiriman baru" : "Edit pengiriman"}
              </h2>
            </div>
            <form onSubmit={submit}>
              <fieldset disabled={busy}>
                ...
                <input
                  ...
                  aria-invalid={!!fieldErrors.orderRef}
                  aria-describedby={
                    fieldErrors.orderRef ? "order-error" : undefined
                  }
                />
                ...
                <textarea
                  ...
                  aria-invalid={!!fieldErrors.destAddress}
            ...
            <div className="form-note">
              ...
              <p>
                Alamat yang sudah dikenali digunakan kembali. Jika koordinat
                belum tersedia, pengiriman tetap tersimpan dengan zona UNKNOWN.
              </p>
            </div>
          </aside>
          <section
          ...
          >
                      ...
                      <div>
                        ...
                        <strong>
                          {delivery.distanceKm === null
                            ? "—"
                            : `${number.format(delivery.distanceKm)} km`}
                        </strong>
                      </div>
                      <div>
                        ...
                        <strong>
                          {delivery.estimate
                            ? money.format(delivery.estimate.costIdr)
                            : "—"}
                        </strong>
                      </div>
                      <div>
                        ...
                        <strong>
                          {delivery.estimate
                            ? `${delivery.estimate.minDays}${delivery.estimate.minDays !== delivery.estimate.maxDays ? `–${delivery.estimate.maxDays}` : ""} hari`
                            : "—"}
                        </strong>
                      </div>
                    </div>
            ...
            <p className="board-footnote">
              Biaya dan waktu tiba adalah simulasi per zona. Jarak bukan jarak
              rute jalan; zona ditentukan sebelum pembulatan jarak.
            </p>
          </section>
        </div>
      </main>
      ...
      >
        ...
        <p>
          <strong>{deleteTarget?.orderRef}</strong> akan dihapus dari daftar.
        ...
        <div className="dialog-actions">
          ...
          <button
            ...
            onClick={remove}
          >
            {pendingId !== null ? "Menghapus…" : "Hapus pengiriman"}
          </button>
        </div>
      </dialog>
    </div>
